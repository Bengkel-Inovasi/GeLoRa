...
export default function SOSButton() {
  ...
  async function handleSend() {
    ...
    setSending(true);
    try {
      await sendAlert(message.trim() || 'Emergency! I need help!');
      setSent(true);
      setMessage('');
      setTimeout(() => {
        setSent(false);
          ...
          <div className="relative bg-white rounded-t-2xl sm:rounded-2xl p-6 w-full max-w-sm mx-0 sm:mx-4 shadow-2xl">
            {sent ? (
              ...
              <>
                ...
                <textarea
                  className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm resize-none focus:outline-none focus:ring-2 focus:ring-red-300 mb-4"
                  rows={3}
                ...
                <div className="flex gap-2">
                  ...
                  <button
                    onClick={handleSend}
                    disabled={sending}
                    className="flex-1 py-2.5 rounded-lg bg-red-600 hover:bg-red-700 disabled:opacity-60 text-sm font-semibold text-white transition-colors"
                  >
